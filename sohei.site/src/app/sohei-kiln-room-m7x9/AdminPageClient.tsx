...
const PAGE_CONTENT_KEYS: Record<string, string[]> = {
  index: [
    'index.heading_philosophy', 'index.philosophy',
    'index.heading_work', 'index.work_intro',
    'index.heading_production', 'index.production_intro',
    'index.heading_interview', 'index.interview_intro',
    'index.heading_artist', 'index.artist_intro',
    'index.heading_stores', 'index.store_0_name', 'index.store_1_name', 'index.store_2_name',
    'index.heading_exhibitions', 'index.exhibition_0_name', 'index.exhibition_1_name', 'index.exhibition_2_name', 'index.exhibition_3_name',
    'index.heading_contact', 'index.contact_text', 'index.copyright',
  ],
  work: [
    'work.heading_h1', 'work.heading_list',
    'work.caption_0', 'work.caption_1', 'work.caption_2', 'work.caption_3',
    'work.caption_4', 'work.caption_5', 'work.caption_6', 'work.caption_7', 'work.caption_8',
    'work.heading_features', 'work.features',
    'work.heading_new_attempts', 'work.new_attempts',
    'work.heading_faq',
    'work.faq_q_0', 'work.faq_0', 'work.faq_q_1', 'work.faq_1',
    'work.faq_q_2', 'work.faq_2', 'work.faq_q_3', 'work.faq_3',
    'work.faq_q_4', 'work.faq_4', 'work.faq_q_5', 'work.faq_5',
  ],
  production: [
    'production.heading_h1', 'production.heading_h2', 'production.byline', 'production.intro',
    'production.step_0', 'production.step_1', 'production.step_2', 'production.step_3',
    'production.step_4', 'production.step_5', 'production.step_6', 'production.step_7',
    'production.step_8', 'production.step_9', 'production.step_10', 'production.step_11',
    'production.step_12', 'production.step_13', 'production.step_14', 'production.step_15', 'production.step_16',
  ],
  interview: [
    'interview.heading_h1',
    'interview.q_0', 'interview.a_0', 'interview.q_1', 'interview.a_1',
    'interview.q_2', 'interview.a_2', 'interview.q_3', 'interview.a_3',
    'interview.q_4', 'interview.a_4', 'interview.q_5', 'interview.a_5',
  ],
  artist: [
    'artist.heading_h1', 'artist.heading_profile', 'artist.profile', 'artist.quote',
    'artist.heading_timeline',
    'artist.timeline_0', 'artist.timeline_1', 'artist.timeline_2', 'artist.timeline_3',
    'artist.timeline_4', 'artist.timeline_5', 'artist.timeline_6',
  ],
};

const PAGE_IMAGE_KEYS: Record<string, string[]> = {
  index: ['index.philosophy_image', 'index.work_image', 'index.production_image', 'index.interview_image', 'index.artist_image'],
  work: ['work.top_image', 'work.image_0', 'work.image_1', 'work.image_2', 'work.image_3', 'work.image_4', 'work.image_5', 'work.image_6', 'work.image_7', 'work.image_8'],
      ...
      <div className="tab-content">
        ...
        <div className="dashboard-section">
          ...
          <div className="stats-grid">
            <div className="stat-card">
              ...
              <div className="stat-value">{stats?.contentStats?.lastUpdated ? new Date(stats.contentStats.lastUpdated).toLocaleDateString('ja-JP') : '-'}</div>
            </div>
            <div className="stat-card">
        ...
        <div className="dashboard-section">
          ...
          <div className="analytics-period-selector">
            {[7, 14, 30].map(d => (
              <button key={d} className={`admin-btn btn-secondary analytics-period-btn ${analyticsPeriod === d ? 'active' : ''}`} onClick={() => setAnalyticsPeriod(d)}>
                {d}日間
              </button>
            ))}
          </div>
            ...
            <>
              ...
              <div className="analytics-chart-container" style={{ marginTop: '16px', marginBottom: '16px' }}>
                ...
                <div className="daily-chart">
                  {Object.entries(stats.daily).sort().slice(-analyticsPeriod).map(([date, data]) => {
              ...
              <div className="visitor-stats" style={{ marginBottom: '16px' }}>
                ...
                <table className="analytics-table">
                  <thead><tr><th>ページ</th><th>アクセス数</th></tr></thead>
                  <tbody>
                    {stats.byPage.map(p => (
                      <tr key={p.page}><td>{p.page}</td><td className="num">{p.count}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
                  ...
                  {Object.entries(stats.devices).map(([device, count]) => {
                    ...
                    return (
                      <div key={device} className="device-bar-row">
                        <span className="device-label">{device === 'mobile' ? 'モバイル' : device === 'tablet' ? 'タブレット' : 'デスクトップ'}</span>
                        <div className="device-bar-track">
                          <div className="device-bar-fill" style={{ width: `${total ? (count / total) * 100 : 0}%` }}></div>
                        </div>
                        <span className="device-count">{count} ({total ? Math.round((count / total) * 100) : 0}%)</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="visitor-stats">
                ...
                <table className="analytics-table">
                  <thead><tr><th>ブラウザ</th><th>アクセス数</th></tr></thead>
                  <tbody>
                    {stats.browsers.map(b => (
                      <tr key={b.browser}><td>{b.browser}</td><td className="num">{b.count}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
    ...
    return (
      <div className="tab-content">
        ...
        <div className="editor-actions">
          <button className="admin-btn btn-primary" onClick={() => handleSave(page)}>保存</button>
          <button className="admin-btn btn-secondary" onClick={() => handleReset(page)}>リセット</button>
        </div>
        ...
        <div className="editor-sections">
          {keys.map(key => {
            ...
            const isLong = key.includes('philosophy') || key.includes('features') || key.includes('new_attempts') || key.includes('step_') || key.includes('intro') || key.includes('.a_') || key.includes('faq_') && !key.includes('faq_q_') || key.includes('quote') || key.includes('profile');
            return (
    ...
    <div className="tab-content">
      ...
      <div className="editor-section">
        ...
        <form onSubmit={handleChangePassword}>
          <input type="password" name="currentPassword" className="settings-input" placeholder="現在のパスワード" required />
          <input type="password" name="newPassword" className="settings-input" placeholder="新しいパスワード" required />
          <div className="settings-actions">
            <button type="submit" className="admin-btn btn-primary">変更する</button>
          </div>
        </form>
      </div>
    </div>
        ...
        <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
          ...
          <nav className="sidebar-nav">
            ...
            {navItems.slice(0, 1).map(item => (
              <button key={item.tab} className={`nav-item ${activeTab === item.tab ? 'active' : ''}`} onClick={() => { setActiveTab(item.tab); setSidebarOpen(false); }}>
